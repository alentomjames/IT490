...
<?php
    ...
    $logFile = '/var/log/database/error.log';
    ...
    while(true) {
        ...
        if ($line !== false ){
            ...
            echo "Read line from Database log: $line\n";
            $logEntry = trim($line);
            // Creating a unique hash for each log entry
            $logHash = md5($logEntry);

            echo "Error recieved from Database Server\n";
            ...
            if (isset($sentLogs[$logHash])) {
                ...
                echo "Message sent from Database to Distrubted Logger: $logJSON\n";
                sendLog($logJSON);
